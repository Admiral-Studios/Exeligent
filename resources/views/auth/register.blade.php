        ...
        <div class="login-section-wrapper">
            ...
            <form id="registrationForm" action="{{ route('register') }}" method="POST">
                ...
                <div class="login-section-box">
                    ...
                    <input class="a-input @error('email') is-invalid @enderror" type="email" name="email"
                           placeholder="Email *" value="{{ old('email') }}" required>
                    @error('email')
                    <span class="invalid-feedback" data-name="email" role="alert">{{ $message }}</span>
                    @enderror

                    <div class="a-inputs-container flex align-center">
                        <div class="a-input-box">
                            <label class="a-label" for="email">First Name</label>
                            <input class="a-input @error('first_name') is-invalid @enderror" type="text"
                                   name="first_name" placeholder="First Name *"
                                   value="{{ old('first_name') }}" required>
                            @error('first_name')
                            <span class="invalid-feedback" data-name="first_name" role="alert">{{ $message }}</span>
                            @enderror
                        </div>

                        <div class="a-input-box">
                            <label class="a-label" for="email">Last Name</label>
                            <input class="a-input @error('last_name') is-invalid @enderror" type="text" name="last_name"
                                   placeholder="Last Name *"
                                   value="{{ old('last_name') }}" required>
                            @error('last_name')
                            <span class="invalid-feedback" data-name="last_name" role="alert">{{ $message }}</span>
                            @enderror
                        </div>
                    </div>

                    @if(request()->input('position'))
                        <input class="a-input @error('position') is-invalid @enderror" type="text" name="position"
                               placeholder="Job Title" value="{{ old('position') }}"
                               required>
                    @error('position')
                        <span class="invalid-feedback" data-name="position" role="alert">{{ $message }}</span>
                        @enderror
                    @endif

                    <label class="a-label" for="email">Password</label>
                    <div class="a-input-eye">
                        <input name="password" required="" id="newPassword"
                               class="a-input a-input-password @error('password') is-invalid @enderror" type="password"
                        ...
                        @error('password')
                        <span class="invalid-feedback" data-name="password" role="alert">{{ $message }}</span>
                        @enderror
                    </div>

                    <label class="a-label" for="email">Confirm Password</label>
                    <div class="a-input-eye">
                        <input name="password_confirmation" required="" id="confirmPassword"
                               class="a-input a-input-password"
                        ...
                        <span class="invalid-feedback" data-name="password_confirmation" role="alert"></span>
                        @enderror
                    </div>

                    {{--                    <div class="login-section-buttons flex align-center">--}}
                    {{--                        <button type="submit" class="btn btn-black fz-024">Sign Up</button>--}}
                    {{--                        <p class="inter fw-regular dark fz-018">Already have an account?--}}
                    {{--                            <a class="fw-semibold" href="{{ route('login') }}">Log In</a>--}}
                    {{--                        </p>--}}
                    {{--                    </div>--}}
            ...
            <div class="register-buttons">
                <button type="submit" form="registrationForm" class="btn btn-blue">Next</button>
            </div>
